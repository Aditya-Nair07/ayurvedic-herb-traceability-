...
async function getOfflineData() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('HerbTraceabilityDB', 1);
    
    request.onsuccess = (event) => {
      const db = event.target.result;
      const transaction = db.transaction(['offlineEvents'], 'readonly');
...
async function removeOfflineData(id) {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = (event) => {
      const db = event.target.result;
      const transaction = db.transaction(['offlineEvents'], 'readwrite');
      const store = transaction.objectStore('offlineEvents');
